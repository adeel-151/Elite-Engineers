...
const ServicesManager = ({ token }) => {
  ...
  return (
    <div>
      ...
      {/* List */}
      <h2 className="text-xl font-display uppercase tracking-widest mb-6 text-gray-900 dark:text-white">Existing Services</h2>
      {loading ? (
        <div className="text-gray-500 dark:text-gray-400 text-sm">Loading services...</div>
      ) : services.length === 0 ? (
        <div className="text-gray-500 dark:text-gray-400 text-sm italic">No services found.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map(service => (
            <div key={service._id} className="border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-primary p-4 flex flex-col hover:border-gray-600 transition-all">
              {service.img && (
                <img src={service.img.startsWith('http') ? service.img : `${API_BASE_URL}/${service.img.replace(/\\/g, '/')}`} alt={service.title} className="w-full h-40 object-cover mb-4 bg-white dark:bg-secondary" />
              )}
              <h3 className="font-display uppercase tracking-widest text-lg mb-2 text-gray-900 dark:text-white">{service.title}</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 line-clamp-3 flex-grow">{service.desc}</p>
              
              <div className="flex gap-2 mt-auto pt-4 border-t border-gray-200 dark:border-gray-800">
                <button onClick={() => handleEdit(service)} className="flex-1 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-700 text-gray-900 dark:text-white text-xs tracking-widest uppercase transition-colors flex justify-center items-center gap-2">
                  <FaEdit /> Edit
                </button>
