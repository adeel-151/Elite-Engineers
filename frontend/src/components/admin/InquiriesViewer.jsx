...
const InquiriesViewer = ({ token }) => {
  ...
  if (loading) return <div className="text-gray-500 dark:text-gray-400 uppercase tracking-widest text-sm">Loading Inquiries...</div>;

  return (
    <div>
      <h2 className="text-xl font-display uppercase tracking-widest mb-8 text-gray-900 dark:text-white">Inquiries & Messages</h2>
      
      <div className="overflow-x-auto border border-gray-200 dark:border-gray-800 rounded-lg">
        <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400">
          <thead className="text-xs text-gray-300 uppercase bg-white dark:bg-secondary border-b border-gray-200 dark:border-gray-800">
            <tr>
          ...
          <tbody>
            {inquiries.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-6 py-8 text-center text-gray-500 italic bg-gray-50 dark:bg-primary">No inquiries found.</td>
              </tr>
            ) : (
              inquiries.map((inq) => (
                <tr key={inq._id} className="bg-gray-50 dark:bg-primary border-b border-gray-200 dark:border-gray-800 hover:bg-white dark:bg-secondary transition-colors">
                  ...
                  <td className="px-6 py-4 max-w-md">
                    {inq.subject && <div className="font-semibold text-gray-900 dark:text-white mb-1">{inq.subject}</div>}
                    ...
                    {(inq.serviceType || inq.area) && (
                      <div className="mt-2 text-xs bg-white dark:bg-secondary p-2 rounded border border-gray-200 dark:border-gray-800">
