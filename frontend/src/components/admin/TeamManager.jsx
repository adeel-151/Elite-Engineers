...
const TeamManager = ({ token }) => {
  ...
  return (
    <div>
      ...
      <h2 className="text-xl font-display uppercase tracking-widest mb-6 text-white">Our Team</h2>
      {loading ? (
        <div className="text-gray-400 text-sm">Loading team...</div>
      ) : team.length === 0 ? (
        <div className="text-gray-400 text-sm italic">No team members found.</div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {team.map(member => (
            <div key={member._id} className="border border-gray-800 bg-primary flex flex-col items-center text-center p-6 relative group hover:border-gray-600 transition-all">
              <div className="w-24 h-24 rounded-full overflow-hidden mb-4 border-2 border-gray-700 bg-secondary">
              ...
              <h3 className="font-display uppercase tracking-widest text-sm mb-1 text-white">{member.name}</h3>
              <p className="text-xs text-accent uppercase tracking-widest">{member.role}</p>
              
              <div className="flex gap-2 w-full mt-6 border-t border-gray-800 pt-4">
                <button onClick={() => handleEdit(member)} className="flex-1 py-1 text-gray-400 hover:text-white text-[10px] tracking-widest uppercase transition-colors flex justify-center items-center gap-1">
                  <FaEdit /> Edit
                </button>
